...
export class WebMWriter {
    ...
    async finish() {
        ...
        const write_metadata = async () => {
            await (<FileSystemWritableFileStream>this.writable).seek(0);
            await (<FileSystemWritableFileStream>this.writable).write(refinedMetadataBuf);

            let void_size = space - refinedMetadataBuf.byteLength;
            if (void_size >= 2) {
                await (<FileSystemWritableFileStream>this.writable).write(new EBML.Encoder().getSchemaInfo('Void'));
                void_size -= 2; // one for element ID (above), one for VINT_WIDTH
                if (void_size < 4) {
                    await (<FileSystemWritableFileStream>this.writable).write(Uint8Array.from([void_size & 0x80]));
                } else {
                    const buf = new ArrayBuffer(5);
                    const view = new DataView(buf);
                    view.setUint8(0, 0b00001000);
                    view.setUint32(1, void_size - 4);
                    await (<FileSystemWritableFileStream>this.writable).write(buf);
                }
            }

            await (<FileSystemWritableFileStream>this.writable).close();
        };
